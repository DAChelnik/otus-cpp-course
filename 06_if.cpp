...
int main(int, char *[]) {
	...
	// 4/2. if, if-else, else without brackets
	if (x < 6 || (0 == x % 2))
		std::cout << "first condition is selected \n";
	else if (x > 1000)
		std::cout << " second condition " << std::endl;
	...
	// 7. if with initializer (since C++17!)
	int part1 = 1;
	part1 += 10;
	int part2 = 2;
	part2 += 100;
	if (const auto sum = part1 + part2; sum > 0) {
		std::cout << "part1 and part2 sum is " << sum << std::endl;
	}

	// 8. constexpr if (since C++17!)
	if constexpr (const auto sum = 10 + 10; sum > 0) {
		std::cout << "sum in is constexpr if is " << sum << std::endl;
	}
